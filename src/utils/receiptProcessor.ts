...
export const processReceipt = async (
  ...
  try {
    ...
    if (isPDF(filePath)) {
      ...
      console.log("[PDF2PNG] PNG pages:", pngPages);
      const pngPath = pngPages[0]?.path;
      if (!pngPath || !fs.existsSync(pngPath)) {
        console.error("[PDF2PNG] PNG page was not created:", pngPath);
        throw new Error("PNG page was not created: " + pngPath);
      }
      // Log PNG file size for debugging
      const stats = fs.statSync(pngPath);
      console.log("[DEBUG] PNG file size:", stats.size);
      const worker = await createWorker('eng');
      const result = await worker.recognize(pngPath);
      console.log("[TESSERACT] OCR text:", result.data.text);
    ...
    // Before returning, log the items array
    console.log("[RECEIPT] Extracted items array:", extractedData.items);
